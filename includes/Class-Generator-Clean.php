...
class Generator {
    ...
    public function generate_summary($post_id, $force = false) {
        ...
        $provider = $this->settings->get_option('ai_provider', 'openrouter');
    ...
    private function call_openrouter_api($content, $api_key, $post_id) {
        $model = $this->settings->get_option('openrouter_model', 'meta-llama/llama-3.1-8b-instruct:free');
        $max_tokens = intval($this->settings->get_option('summary_max_tokens', 150));
        
        $prompt = "Please provide a concise summary of the following text in " . 
                 $this->settings->get_option('summary_language', 'English') . ". " .
    ...
    private function call_gemini_api($content, $api_key, $post_id) {
        $model = $this->settings->get_option('gemini_model', 'gemini-1.5-flash');
        $max_tokens = intval($this->settings->get_option('summary_max_tokens', 150));
